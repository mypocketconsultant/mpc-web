"use client";


import React, { useState, useEffect } from "react";
import { usePathname, useRouter } from "next/navigation";
import Link from "next/link";
import { ChevronLeft, Settings as SettingsIcon , ArrowLeft  } from "lucide-react";
import SettingsHeader from "@/app/components/settingsHeader";
import SortIcon from "@/public/icons/Sort";
import VolunteerList from "../components/VolunteerList";
import CustomMap from "../components/CustomMap";
import Modal from "@/app/components/Modal";
import { X } from 'lucide-react';

const page = () => {
  
   const pathname = usePathname();
  
    const router = useRouter();
     const [isOpen, setIsOpen] = useState(false);
 
  return (
    <div className="flex flex-col h-full">
       <SettingsHeader title="Social Impact" />
       <main className="flex-1 overflow-auto max-w-[1200px] mx-auto scrollbar-hide w-full">
        <div  className="max-w-7xl mx-auto px-3 sm:px-4 md:px-6 py-4 sm:py-6 md:py-8">
                <div className="flex  flex-row items-center justify-between my-3 sm:my-6">
            <Link href="/modules/social-impact">
              <button className="flex items-center gap-2 text-xs sm:text-sm text-gray-700 hover:text-[#5A3FFF] transition-colors cursor-pointer">
                <ArrowLeft className="h-3 w-3 sm:h-4 sm:w-4" />
                <span>Social Impact/Volunteer Project Finder</span>
              </button>
            </Link>
          </div>
           <hr className="my-4 sm:my-10" />
          <div className="flex flex-row justify-between items-center">
            <div className="flex flex-row items-center gap-2">
                <SortIcon/>
            <p className="font-medium text-[#000000]">Sort by</p>
            </div>
              <div className="border border-[#CFCFCF] flex flex-row items-stretch rounded-xl md:rounded-md overflow-hidden shrink-0 text-[10px] sm:text-xs md:text-sm">
  {/* Left Side (Drafts) */}
   <div className="bg-[#EDE6FF] px-2 py-1 md:px-3 md:py-1.5 flex items-center cursor-pointer"
    onClick={() => router.push('/modules/social-impact/volunteer-list')}
   >
    <p className="text-[#3566A2] font-bold whitespace-nowrap">
      List <span className="hidden sm:inline"> view</span>
    </p>
  </div>
  
  
  {/* Right Side (Published) */}
 <div className="bg-white px-2 py-1 md:px-3 md:py-1.5 flex items-center  border-r border-[#CFCFCF] cursor-pointer"
  onClick={() => router.push('/modules/social-impact/Custom-map')}
 >
    <p className="text-[#16375F] whitespace-nowrap">
     Map
    </p>
  </div>
</div>
          </div>
          <div className="flex flex-row items-center flex-wrap justify-between mt-2">
            <div >
              <VolunteerList visible={()=> setIsOpen(true)} />
            </div>
            <div >
              <CustomMap/>
            </div>
          </div>
             

                         
        </div>
       </main>
        <Modal isOpen={isOpen} onClose={() => setIsOpen(false)}>
      <div className="py-8 px-12">
       <div
            className="absolute top-4 right-4 text-gray-500 hover:text-gray-800 bg-[#E8E8E8] rounded-full p-2"
            onClick={() => setIsOpen(false)}
          >
            <X  color="#000000"/>
          </div>
        <div className="mt-12">
          <div className="w-full bg-[#FCFCFF] p-4 rounded-2xl shadow-[0_8px_30px_rgb(0,0,0,0.08)] border border-gray-100/50">
            <div className="flex justify-end items-center w-full h-full mt-2">
  <button
    className=" flex items-center justify-center px-2 py-2 w-[60px] rounded-[10px] cursor-pointer hover:opacity-90 active:scale-95 transition-all bg-[conic-gradient(from_236.3deg_at_92.05%_3.9%,_#300878_-5.19deg,_#5A3FFF_133.27deg,_#D4AF37_254.42deg,_#300878_354.81deg,_#5A3FFF_493.27deg)]"
    onClick={() => setIsOpen(true)}
  >
    <p className="text-xs sm:text-base text-white m-0 font-medium">Send</p>
  </button>
          </div>
          <div className="">
            <p className='text-[#062950] font-medium'>Volunteers at Woodhaven Blvd</p> 
            <p className='pt-1'>📓 <span className='text-[#300878] font-bold '>Education</span></p> <p className='text-[#000000] font-regular pt-0.5'>4 hours/week</p>
             <p className='text-[#062950] font-medium'>Remote</p>
          </div>
            <p className='text-[#062950] font-medium py-1'>To apply:<br />
Send mail to volunteer@rego.park<br />
OR<br />
Visit website www.rego.park</p>
          <div>
          </div>
          </div>
     
        </div>
       </div>
      </Modal>
    </div>
  )
}

export default page